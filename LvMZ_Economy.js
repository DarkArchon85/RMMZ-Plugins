 ...
 * NOTE: If VisuStella's Core plugin is present, you can ignore this.
 *
 *
 * >> STOLEN ITEMS <<
 *
 ...
 *   VARIES - Only shops with <StolenItems: Allow> tag to buy them.
 ...
 * @decimals 0
 * @min 0
 * @desc The icon to display in menues for stolen items.
 * @default 19
 *
 * @param stolenValue
...
// WARNING: Using this method while gear is stored, will OVERWRITE!
// * recover{boolean}: Can reset stolen item events in the world
Game_Party.prototype.storeGear = function(recover) {
	...
	// - Then we store everything -
	this._storage[0] = this._items;
	this._storage[1] = this._weapons;
	this._storage[2] = this._armors;
	this._storage[3] = this._gold;
...
Window_ShopBuy.prototype.refresh = function() {
	...
	if (this._data.length === 0) SceneManager.pop();
};
...
LordV_Shop.prototype.checkPrice = function(item) {
	...
	if (!Imported['LvMZCurrencies']) return item.price > 0;
