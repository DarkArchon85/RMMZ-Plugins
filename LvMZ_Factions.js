// ============================================================================
//  LordValinar Plugin - Factions, Genders, Races, Fame, Relations and Titles
//  LvMZ_Factions.js
// ============================================================================
...
if (!Imported['LvMZ_Core']) {
	...
	MapManager.simulateEvents = function(mapId) {
		...
		let list = [];
		this.loadMapData(mapId, result => {
			list = result.events.filter(ev => !!ev);
		});
	...
	// -- GAME PLAYER (Extended Activation)
	Game_Player.prototype.extendActivate = function(length = 1) {
		if (this.canStartLocalEvents()) {
			const d = this.direction();
			let x = $gameMap.roundXWithDirection(this._x, d);
			let y = $gameMap.roundYWithDirection(this._y, d);
			while (--length >= 0) {
				...
				if (!$gameMap.isValid(x1, y1)) {
					alert("Error: extendActivate - Out of bounds!");
					console.log("DEBUG :: Game_Player :: extendActivate");
	...
	LvMZ_RemoteEvent.prototype.event = function() {
		...
		let data = null;
		MapManager.loadMapData(this._mapId, result => {
			data = result.events[eventId];
		});
		return data;
	};
 ...
 *     Type: Faction | Gender | Race | Reputation | Relation | Title
 ...
 * ----------------------------------------------------------------------------
 *
 * v1.7 - Added reputation(aka the "Fame" system)
...
Game_Actor.prototype.initFactionSettings = function() {
	...
	const tag = /<(FACTION|RACE|TITLE|GENDER)[: ]+([^>]+)>[: ]*(LEADER)?/i;
...
Game_Event.prototype.meetsConditions = function(page) {
	...
	if (c.variableValid) {
		name = v[c.variableId];
		if (name.match(/\[(RELATION|ROMANCE|REPUTATION)\]/i)) {
...
Game_Event.prototype.initPricePlus = function() {
	if (!this._DLL) return;
	const data = ["faction","race","gender","reputation","relation","romance","title"];
...
Game_Factions.prototype.modifyVsPrice = function(source, target, value) {
...
Game_Factions.prototype.setRelation = function(source, target, value) {
	...
	const name = target instanceof Game_Actor ? target.name() : target.event().name;
...
Game_Factions.prototype.friends = function(source) {
	const data = source._DLL.relations;
	return this.relateList(data);
};

Game_Factions.prototype.romanced = function(source) {
	const data = source._DLL.romances;
	return this.relateList(data);
};
...
Game_Factions.prototype.relateList = function(data) {
	...
	const mapList = $dataMapInfos.filter(e => !!e).map(map => Number(map.id));
...
Game_Factions.prototype.priceAdjust = function(source, target, type) {
	const sourceName = this.getSource(source, type);
	const targetName = this.getTarget(source, target, type);
	const data = this.compare(type, sourceName);
	if (data) {
		const adj = this.pricePlus(source, type, targetName);
		const price = data.vs ? data.vs[targetName] || 0 : data.price;
...
Game_Factions.prototype.getTarget = function(source, target, type) {
	switch (type.toLowerCase()) {
		...
		case 'relation': return this.relationName(source, target);
		case 'romance': return this.romanceName(source, target);
		case 'title': return this.lastTitle(target);
	}
	return "";
};

Game_Factions.prototype.compare = function(type, value) {
	let list = this.data(type);
	switch (type.toLowerCase()) {
		case 'faction': case 'race': case 'gender': 
		case 'reputation': case 'title': 
			list = list.filter(e => e.name === value);
			break;
		case 'relation': case 'romance': 
			list = list.filter(e => Number(value) >= e.min);
			list = list.filter(e => Number(value) <= e.max);
			break;
	}
...
Game_Factions.prototype.min = function(type) {
	const list = ["reputation","relation","romance"];
